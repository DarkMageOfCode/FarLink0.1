...
namespace FarLink.RabbitMq.Internals
{
        ...
        where TEvent : IEvent
    {
        ...
        public EventPublisher(ProducerDictionary producers, ISerializationService serializationService, IMetadataCache metadata,
        ...
        public IEventPublisher<TEvent> MessageId(string messageId)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, _confirmsMode, _messageTtl, _deliveryMode,
                _timeout, _correlationId, messageId);
            

        public IEventPublisher<TEvent>  CorrelationId(string correlationId)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, _confirmsMode, _messageTtl, _deliveryMode, _timeout,
                correlationId, _messageId);
        
        public IEventPublisher<TEvent>  Persistent(bool persistent)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, _confirmsMode, _messageTtl, persistent ? LinkDeliveryMode.Persistent : LinkDeliveryMode.Transient, _timeout,
                _correlationId, _messageId);
        
        public IEventPublisher<TEvent> ConfirmPublish(bool confirmPublish)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, confirmPublish, _messageTtl, _deliveryMode, _timeout,
                _correlationId, _messageId);

        public IEventPublisher<TEvent> MessageTtl(TimeSpan? messageTtl)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, _confirmsMode,
        ...
        public IEventPublisher<TEvent> Timeout(TimeSpan timeout)
            => new EventPublisher<TEvent>(_producers, _serializationService, _metadata, _confirmsMode, _messageTtl, _deliveryMode, timeout,
                _correlationId, _messageId);
        ...
        public async Task PublishAsync(TEvent message, CancellationToken cancellation = default)
        {
            ...
            var contentTypes = _metadata.GetEventAttributes<ContentTypeAttribute>(eventType)?.OrderBy(p => p.Priority).ToList();
